...
class RegionLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        # cls = cls.view(nB * nA, nC, nH * nW)
        # cls = cls.transpose(1,2).contiguous()
        # cls = cls.view(nB*nA*nH*nW, nC)
        ...
        pred_boxes = convert2cpu(pred_boxes.transpose(0, 1))
        ...
        loss_x = self.coord_scale * nn.MSELoss(size_average=False)(x*coord_mask, (tx*coord_mask))/2
        loss_y = self.coord_scale * nn.MSELoss(size_average=False)(y*coord_mask, (ty*coord_mask))/2
        loss_w = self.coord_scale * nn.MSELoss(size_average=False)(w*coord_mask, (tw*coord_mask))/2
        loss_h = self.coord_scale * nn.MSELoss(size_average=False)(h*coord_mask, (th*coord_mask))/2
